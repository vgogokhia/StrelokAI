...
const server = app({ db, origin, clientId: process.env.GOOGLE_CLIENT_ID, clientSecret: process.env.GOOGLE_CLIENT_SECRET, webRoot: process.env.WEB_ROOT || '../web/dist', adminEmails: (process.env.ADMIN_EMAILS || '').split(','),
  assistant: { keys: { anthropic: process.env.ANTHROPIC_API_KEY, openai: process.env.OPENAI_API_KEY, gemini: process.env.GEMINI_API_KEY }, model: process.env.ASSISTANT_MODEL, dailyLimit: Number(process.env.ASSISTANT_DAILY_LIMIT || 30) },
