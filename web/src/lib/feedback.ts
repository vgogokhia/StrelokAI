...
export async function sendFeedback(payload: { kind: string; message: string; contact: string; meta: Record<string, unknown>; screenshot?: string | null }) {
  try {
  ...
  } catch { return { ok: false as const, error: "offline" }; }
}
